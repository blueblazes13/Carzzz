...
public class CarLabelsFXMLController {
    ...
    void initialize() {
        ...
        this.btnNewLabel.setOnMouseClicked((MouseEvent me) -> {
            if (tfLabelName.getText() != "") {
                CarzzzModel.infoLabels.addCarLabel(this.tfLabelName.getText());
                this.setLabels();
            }
        });
        
        ScreenController.addListener("CarLabelsFXML", this::close);
    }
    
    
    private void close(String fxml) {
        ScreenController.closeScreen("CarLabelsFXML");
    }
    
    
    private void setLabels() {
        this.VBoxLabels.getChildren().clear();
        for (String label: CarzzzModel.infoLabels.getCarLabels()) {
    ...
    private void removeLabel(String label) {
        CarzzzModel.infoLabels.removeCarLabel(label);
